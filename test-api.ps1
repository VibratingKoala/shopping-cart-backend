...
try {
    ...
    Write-Host "   Total: $($cart.total.amount) $($cart.total.currency)" -ForegroundColor Cyan
} catch {
...
try {
    ...
    Write-Host "✗ Add item failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "✓ Cart with items retrieved successfully" -ForegroundColor Green
    Write-Host "   Cart ID: $($cartWithItems.id)" -ForegroundColor Cyan
    Write-Host "   Items: $($cartWithItems.items.Count)" -ForegroundColor Cyan
    Write-Host "   Total: $($cartWithItems.total.amount) $($cartWithItems.total.currency)" -ForegroundColor Cyan
    ...
    Write-Host "✗ Get cart with items failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "✓ Second item added successfully" -ForegroundColor Green
} catch {
    Write-Host "✗ Add second item failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "   Total: $($checkoutResult.total.amount) $($checkoutResult.total.currency)" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "✓ Cart after checkout retrieved" -ForegroundColor Green
    Write-Host "   Items: $($emptyCart.items.Count)" -ForegroundColor Cyan
    Write-Host "   Total: $($emptyCart.total.amount) $($emptyCart.total.currency)" -ForegroundColor Cyan
    
    if ($emptyCart.items.Count -eq 0) {
        Write-Host "✓ Cart is empty as expected after checkout" -ForegroundColor Green
    } else {
        Write-Host "✗ Cart still has items after checkout" -ForegroundColor Red
    }
} catch {
    Write-Host "✗ Get cart after checkout failed: $($_.Exception.Message)" -ForegroundColor Red
}
